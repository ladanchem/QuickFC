...
class QuickFC(ctk.CTk):
    ...
    def flash_redraw(self):
        ...
        if self.flash_mode.get() == 'known':
            ...
            self.D = add_field('D', 'D, the inner diameter of the column (mm)')
            self.L = add_field('L', 'L, the length of the packed silica / the length of the column (cm)')
        else:
            self.m = add_field('m', 'm, the mass of the crude sample to be purified (mg)')
            self.dRf = add_field('dRf', 'dRf, the difference between the Rf values of the purifying compound and and nearest one')
            self.Rfmin = add_field('Rfmin', 'Rfmin, the lowest Rf value of the purifying compound based on TLC (between 0.01 and 0.99)')
            self.L = add_field('L', 'L, the length of the packed silica / the length of the column (if unknown, enter average value ≈25) (cm)')
    ...
    def prepare_redraw(self):
        ...
        if self.major.get() == 'new':
            ...
            if self.minor.get() == 'pea':
                ...
                self.v = add_field('V','V, total volume of the preparing eluent (ml)')
            else:
            ...
            vol_frame.pack(side='left', fill='x', expand=True, padx=(6,0))
            self.vol = ctk.CTkEntry(vol_frame, placeholder_text='Target V')
            self.vol.pack(fill='x', pady=(3,0))
            info = ctk.CTkButton(row,text='ⓘ',width=25,height=25,corner_radius=15)
            info.pack(side='right', padx=5)
            CTkToolTip(info,message='Target V, the volume of the preparing eluent (ml)',delay=0.1)
            self.minv = ctk.BooleanVar()

            def tgl():
                if self.minv.get():
                    self.vol.pack_forget()
                else:
                    self.vol.pack(fill='x',pady=(3,0))
            ctk.CTkCheckBox(row,text='Minimum',variable=self.minv,command=tgl).pack(side='right',padx=(0,0),pady=(4,0))
    ...
    def calc_prepare(self):
        ...
        if self.major.get()=='new':
            if self.minor.get()=='pea':
                ...
                ea_result=v*e/(p+e)
                self.prepare_out.configure(font=ctk.CTkFont(family="Arial", size=15)); self.prepare_out.delete('1.0','end');
                self.prepare_out.insert('end',f'\n\n\n\n\n\nThe composition of your eluent:\n\nPentane: {pentane_result:.0f}ml\nEthylacetate: {ea_result:.0f}ml')
            else:
                v=float(self.cv.get()); total=sum(float(p.get()) for _, p in self.custom_rows if p.get())
                self.prepare_out.configure(font=ctk.CTkFont(family="Arial", size=15)); self.prepare_out.delete('1.0','end');
                self.prepare_out.insert('end', '\n\n\n\n\n\nThe composition of your eluent:\n')
                for n,p in self.custom_rows:
                    if n.get() and p.get():
                        parts = v*float(p.get())/total
                        self.prepare_out.insert('end', f'{n.get()}: {parts:.0f}ml\n')
        else:
            ...
            if self.minv.get():
                ...
                self.prepare_out.insert('end',f'You have:\nPentane:')
                self.prepare_out.insert('end',f'\n\n\n\n\n\nAdd:\n\n Pentane: {pent:.0f}\nEA: {ea:.0f}')
